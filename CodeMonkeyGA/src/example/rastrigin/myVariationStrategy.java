...
public class myVariationStrategy extends VariationStrategy {
	...
	public <P extends Phenotype> Population mate(P one, P two) {
		...
		if( chance <0.7) {
	    	...
	    	result[0].recombineRealListConvex(result[1],0.8  );
	    	result[0].mutateRealListCreep(0.2  );
	    	result[1].mutateRealListCreep(0.2  );

		} else if( chance <1.0) {
	    	result[0].recombineRealListContinuous(result[1],0.8  );
