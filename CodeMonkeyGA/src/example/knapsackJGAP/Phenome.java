...
public class Phenome extends Phenotype<myGenome, Double> {
	...
	public void evaluate(HashMap<String,String> externalData) {
		...
		external.KnapsackFitnessFunction evaluator = new external.KnapsackFitnessFunction(200); // knapsack volume
		fitnessValue = evaluator.evaluate(this.genome);
		return; 
	}
	...
	public String express() {
		...
		for (int i=0 ; i<genome.size() ; i++) {
			sb.append(genome.get(i)+" "+KnapsackFitnessFunction.itemNames[i]+", ");
		}
		return sb.toString();
	}
