package example.fullyDeceptive;
...
public class myVariationStrategy extends VariationStrategy {
	...
	public <P extends Phenotype> Population mate(P one, P two) {
		...
		if( chance <0.7) {
	    	...
	    	result[0].recombineIntegerListOnePoint(result[1],0.8 );
	    	result[0].mutateIntegerListOnePosition(0.2  );
	    	result[1].mutateIntegerListOnePosition(0.2  );
		} else if( chance <0.95) {
	
	    	result[0].recombineIntegerListTwoPoint(result[1],0.8  );
	    	result[0].mutateIntegerListCreep(0.2  );
	    	result[1].mutateIntegerListCreep(0.2  );
		}
