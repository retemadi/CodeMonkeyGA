package example.ImageProcessing;
...
public class myVariationStrategy extends VariationStrategy {
	...
	public <P extends Phenotype> Population mate(P one, P two) {
		...
		if( chance <0.3) {
	    	...
	    	result[0].mutateInsert(0.1); //0.1 //0.2 //0.3 //0.8
	    	result[1].mutateInsert(0.1); //0.1 //0.2 //0.3 //0.8

	    	limit = (result[0].size() < result[1].size())?result[0].size():result[1].size();
	    	
			for (int i=0; i<limit; i++) {
	    		((myGenomePart0)result[0].get(i).get(0)).
	    		recombineIntegerListTwoPoint((myGenomePart0)result[1].get(i).get(0),0.9); //0.7
			}
			for (int i=0; i<result[0].size(); i++) {
	    		((myGenomePart0)result[0].get(i).get(0)).mutateIntegerListOnePosition(0.05); //0.1 //0.3 //0.6
			}
			for (int i=0; i<result[1].size(); i++) {
	    		((myGenomePart0)result[1].get(i).get(0)).mutateIntegerListOnePosition(0.05); //0.3 //0.6
			}
		} else if( chance <1.0) {

	    	result[0].recombineListUniform(result[1],0.9); //0.6
	    	result[0].mutateDelete(0.1); //0.4 //0.9
	    	result[1].mutateDelete(0.1); //0.4 //0.9
	    	limit = (result[0].size() < result[1].size())?result[0].size():result[1].size();
	    	
			for (int i=0; i<limit; i++) {
	    		((myGenomePart0)result[0].get(i).get(0)).
	    		recombineIntegerListDiscrete((myGenomePart0)result[1].get(i).get(0),0.9); //0.6
			}
			for (int i=0; i<result[0].size(); i++) {
	    		((myGenomePart0)result[0].get(i).get(0)).mutateIntegerListCreep(0.05); //0.3 //0.6
			}
			for (int i=0; i<result[1].size(); i++) {
	    		((myGenomePart0)result[1].get(i).get(0)).mutateIntegerListCreep(0.05); //0.3 //0.6
			}
